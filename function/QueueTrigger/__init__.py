...
def main(msg: func.ServiceBusMessage):
    ...
    conn = psycopg2.connect(dbname="techconfdb", 
                            user="mnmuproj3@mnmup3server", 
                            password="m@nsi9876", 
                            host="mnmup3server.postgres.database.azure.com")
    cur = conn.cursor()
    try:
        ...
        for (email, first_name) in attendees:
            mail = Mail(
                from_email='ladmansi7@gmail.com',
                to_emails= email,
                subject= subject,
                plain_text_content= "Hi {}, \n {}".format(first_name, body))
            try:
                SENDGRID_API_KEY = "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"
